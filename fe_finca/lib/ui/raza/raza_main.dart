...
import 'package:asistencia_app/ui/raza/MyAppState.dart';
...
import 'package:asistencia_app/ui/raza/raza_edit.dart';
import 'package:asistencia_app/ui/raza/raza_form.dart';
...
class MainRaza extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    /*return Provider<RazaApi>(
      create: (_) => RazaApi.create(),
      child: MaterialApp(
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_)=>RazaBloc(
            RazaRepository())),
      ],
      child: MaterialApp(
        debugShowCheckedModeBanner: false,
...
class RazaUI extends StatefulWidget {
  @override
  _RazaUIState createState() => _RazaUIState();
}

class _RazaUIState extends State<RazaUI> {
  //ApiCovid apiService;
  ...
  void initState() {
    ...
    BlocProvider.of<RazaBloc>(context).add(ListarRazaEvent());
    print("entro aqui");
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => RazaForm()),
                  ).then(onGoBack);
                },
        ...
        body: BlocBuilder<RazaBloc,RazaState>(
          builder: (context,state){
            if(state is RazaLoadedState){
              return _buildListView(context, state.RazaList);
            }else{
              return Center(
        ...
        /*body: FutureBuilder<List<RazaModelo>>(
          future: Provider.of<RazaApi>(context, listen: true)
              .getRaza(TokenUtil.TOKEN).then((value) => value),
          builder: (BuildContext context,
              AsyncSnapshot<List<RazaModelo>> snapshot) {
            if (snapshot.hasError) {
              ...
              List<RazaModelo> persona = snapshot.data!!;
  ...
  Widget _buildListView(BuildContext context, List<RazaModelo> persona) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),
      child: ListView.builder(
        itemBuilder: (context, index) {
          RazaModelo personax = persona[index];
          return Padding(
                  ...
                  children: <Widget>[
                    ListTile(
                        ...
                        title: Row(
                          children: [
                            Container(
                              child: Text(personax.nombreRaza,
                                        ...
                                        onPressed: () {
                                          showDialog(
                                            ...
                                            if (value.toString() == "Success") {
                                              ...
                                              BlocProvider.of<RazaBloc>(context).add(DeleteRazaEvent(personax));
                                            }
                                          });
                                        }))
                                  ],
  ...
  void exportAsistenciaToExcel(List<AsistenciapaxModelo> asistencia) {
    ...
    for (var row = 0; row < asistencia.length; row++) {
      ...
      sheetObject
          ...
          .value = asistenciax.raza.nombreRaza;
      sheetObject
